...
class Kassa:
    ...
    def register_train(self, train):
        for u in self.train
            if u.train == source and x.destination == destination and x.passenger_iin == iin:
                return x
            #
    ...
    def buy_ticket(self, source, destination, person):
        #
        money = person.pay( self.get_price(source, destination), self.register_train(self, train))
        if money:
            self.balance += money
